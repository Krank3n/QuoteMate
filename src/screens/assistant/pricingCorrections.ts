...
export function correctionsClause(lines: string[], quoteId?: string): string {
  ...
  return (
    ...
    `where that's what they meant — a bare phone number or email is the customer's contact: propose_update_customer with just email or phone and no customerId, never scope). ` +
    `Never draft a new quote for them. A line that isn't about this job — small talk, a supplies run, ` +
