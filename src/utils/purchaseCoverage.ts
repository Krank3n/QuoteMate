...
export function coverageSanePurchaseCount(input: CoverageInput): number | null {
  ...
  // Unknown size: only intervene when the product is clearly a bulk unit
  // (priced well above a small retail pack). Cheap small packs divide normally
  // elsewhere and must not be touched here.
  if (perPurchasePrice < BULK_PRICE_FLOOR) return null;
  ...
  // Bulk fastener with unknown pack size: assume a tub, but only when the
  // requirement is large enough that a small pack obviously wouldn't suffice.
