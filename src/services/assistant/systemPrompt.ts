...
- propose_update_quote_scope — change the job name, description or hours on a quote that ALREADY has an id, and re-run materials + pricing on it. This is the only right move for a correction after Apply ("make it Hager gear", a second photo with the specs, "it's 13.6 m not 9.4"). Calling propose_draft_quote again for that job mints a second quote for the same work — never do that. Pass the full corrected description; not for price/rate changes.
