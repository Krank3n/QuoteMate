...
export const TOOL_DECLARATIONS: GeminiFunctionDeclaration[] = [
  ...
  },
  {
    ...
    description:
      "Remember a STANDING rule about how this tradie quotes, in their words, as one plain sentence: \"labour separate from materials\", \"customers supply their own materials\", \"we only quote labour\", \"always round the quote up to a round figure\". Apply saves it to their settings and it applies to every quote from then on — on this phone and every other. Only for rules that outlast this job, never for this job's details. Propose it once when they state one; if they knock the card back, drop it.",
    parameters: {
      type: 'object',
      properties: {
