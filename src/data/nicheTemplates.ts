...
export const NICHE_TEMPLATES: NicheJobTemplate[] = [
  ...
  // ============================================
  {
    id: 'kitchen_cabinetry',
    categoryId: 'cabinet_making',
    nicheId: 'kitchens',
    name: 'Kitchen Cabinetry',
    description: 'Custom kitchen cabinets + benchtop install',
    icon: 'silverware-fork-knife',
    pricingMethod: 'per_linear_m',
    requiredParams: [
      { key: 'linearMetres', label: 'Cabinet Linear Metres', unit: 'lm', defaultValue: 6 },
      { key: 'islandLm', label: 'Island Linear Metres', unit: 'lm', defaultValue: 0 },
    ],
    defaultMaterials: [
      { name: 'White Melamine Carcass Board', searchTerm: 'White melamine board 2400 x 600 x 16mm', quantityFormula: 'Math.ceil((linearMetres + islandLm) * 4)', unit: 'each' },
      { name: 'Polytec Door Panels', searchTerm: 'Polytec laminated door panel custom size', quantityFormula: 'Math.ceil((linearMetres + islandLm) * 2.5)', unit: 'each' },
      { name: 'Blum Soft Close Hinges', searchTerm: 'Blum CLIP top BLUMOTION 110 hinge soft close', quantityFormula: 'Math.ceil((linearMetres + islandLm) * 6)', unit: 'each' },
      { name: 'Blum TANDEMBOX Drawer Runners', searchTerm: 'Blum TANDEMBOX drawer runner 500mm', quantityFormula: 'Math.ceil((linearMetres + islandLm) * 2)', unit: 'each' },
      { name: 'Cabinet Handles', searchTerm: 'Cabinet handle 128mm brushed nickel', quantityFormula: 'Math.ceil((linearMetres + islandLm) * 5)', unit: 'each' },
      { name: 'Edging Tape', searchTerm: 'Iron on edging tape white 22mm x 50m', quantityFormula: 'Math.ceil((linearMetres + islandLm) / 4)', unit: 'each' },
      { name: 'Engineered Stone Benchtop', searchTerm: 'Engineered stone benchtop 20mm supply and install per lm', quantityFormula: 'linearMetres + islandLm', unit: 'm' },
      { name: 'Cabinet Screws', searchTerm: 'Cabinet screws 4mm x 30mm 1000 pack', quantityFormula: 'Math.ceil((linearMetres + islandLm) / 4)', unit: 'pack' },
      { name: 'Confirmat Screws', searchTerm: 'Confirmat screw 7mm x 50mm 100 pack', quantityFormula: 'Math.ceil((linearMetres + islandLm) / 2)', unit: 'pack' },
      { name: 'Adjustable Cabinet Legs', searchTerm: 'Adjustable cabinet leg 100-150mm', quantityFormula: 'Math.ceil((linearMetres + islandLm) * 4)', unit: 'each' },
      { name: 'Kickboard', searchTerm: 'Kickboard matching finish 100mm x 3.6m', quantityFormula: 'Math.ceil((linearMetres + islandLm) / 3)', unit: 'each' },
      { name: 'Silicone Sealant', searchTerm: 'Silicone sealant neutral cure 300ml', quantityFormula: 'Math.max(2, Math.ceil((linearMetres + islandLm) / 3))', unit: 'each' },
    ],
    estimatedHoursFormula: '(linearMetres + islandLm) * 6',
    estimatedHoursRange: { min: 24, max: 80 },
    suggestedMaterials: ['Melamine carcass board', 'Door panels', 'Blum hinges', 'Drawer runners', 'Handles', 'Edging tape', 'Stone benchtop', 'Cabinet screws', 'Kickboard'],
    questionsLine: 'Linear metres of cabinets? Island? Door finish (Polytec, 2-pac)? Benchtop (stone, laminate)? Appliances customer-supplied?',
    exampleLines: '5.4 lm L-shape kitchen plus 2.4 lm island. Polytec Woodmatt doors, soft-close Blum hardware, 20mm Caesarstone benchtop with waterfall end on island. Demo old kitchen, integrated dishwasher, customer-supplied oven and cooktop.',
    voicePromptHint: 'Tell me about the kitchen — linear metres, doors, benchtop, appliances',
  },
  {
    id: 'walk_in_wardrobe',
    categoryId: 'cabinet_making',
    ...
    description: 'Custom walk-in wardrobe fitout',
    icon: 'wardrobe',
    pricingMethod: 'per_linear_m',
    requiredParams: [
      { key: 'linearMetres', label: 'Wardrobe Linear Metres', unit: 'lm', defaultValue: 6 },
      { key: 'height', label: 'Internal Height', unit: 'm', defaultValue: 2.4 },
    ],
    defaultMaterials: [
      { name: 'White Melamine Board', searchTerm: 'White melamine board 2400 x 600 x 16mm', quantityFormula: 'Math.ceil(linearMetres * height * 0.8)', unit: 'each' },
      { name: 'Hanging Rails Chrome', searchTerm: 'Wardrobe hanging rail chrome 25mm 1.8m', quantityFormula: 'Math.ceil(linearMetres * 0.6)', unit: 'each' },
      { name: 'Hanging Rail Brackets', searchTerm: 'Hanging rail bracket chrome', quantityFormula: 'Math.ceil(linearMetres * 1.2)', unit: 'each' },
      { name: 'Blum TANDEMBOX Drawer Runners', searchTerm: 'Blum TANDEMBOX drawer runner 450mm', quantityFormula: 'Math.ceil(linearMetres)', unit: 'each' },
      { name: 'Drawer Fronts', searchTerm: 'Drawer front melamine custom', quantityFormula: 'Math.ceil(linearMetres)', unit: 'each' },
      { name: 'Shelf Pins', searchTerm: 'Shelf support pins nickel 5mm 100 pack', quantityFormula: '1', unit: 'pack' },
      { name: 'Edging Tape', searchTerm: 'Iron on edging tape white 22mm x 50m', quantityFormula: '1', unit: 'each' },
      { name: 'Cabinet Screws', searchTerm: 'Cabinet screws 4mm x 30mm 500 pack', quantityFormula: '1', unit: 'pack' },
      { name: 'Confirmat Screws', searchTerm: 'Confirmat screw 7mm x 50mm 100 pack', quantityFormula: 'Math.ceil(linearMetres / 3)', unit: 'pack' },
      { name: 'Handles', searchTerm: 'Cabinet handle 128mm brushed nickel', quantityFormula: 'Math.ceil(linearMetres * 2)', unit: 'each' },
    ],
    estimatedHoursFormula: 'linearMetres * height * 2.5',
    estimatedHoursRange: { min: 10, max: 36 },
    suggestedMaterials: ['Melamine board', 'Hanging rails', 'Drawer runners', 'Drawer fronts', 'Shelf pins', 'Edging tape', 'Handles'],
    questionsLine: 'Internal linear metres? Hanging vs shelving split? Drawer count? Open shelving or doors?',
    exampleLines: '6 lm U-shape walk-in robe in master, 2.4m high. Two double-hang sections, one bank of 5 drawers with Blum soft-close runners, balance shelving. Open style — no doors. White melamine internals.',
    voicePromptHint: 'Tell me about the walk-in — linear metres, hanging, drawers, doors',
  },
  {
    id: 'bathroom_vanity',
    categoryId: 'cabinet_making',
    nicheId: 'vanities_laundry',
    name: 'Bathroom Vanity',
    description: 'Custom bathroom vanity with stone top',
    icon: 'shower',
    pricingMethod: 'fixed',
    requiredParams: [
      { key: 'widthMm', label: 'Vanity Width', unit: 'mm', defaultValue: 1200 },
      { key: 'basins', label: 'Number of Basins', unit: '', defaultValue: 1 },
    ],
    defaultMaterials: [
      { name: 'White Melamine Carcass', searchTerm: 'White melamine board 2400 x 600 x 16mm', quantityFormula: 'Math.ceil(widthMm / 600)', unit: 'each' },
      { name: 'Polytec Door / Drawer Fronts', searchTerm: 'Polytec laminated door panel custom size', quantityFormula: 'Math.max(2, Math.ceil(widthMm / 400))', unit: 'each' },
      { name: 'Blum Soft Close Hinges', searchTerm: 'Blum CLIP top BLUMOTION 110 hinge soft close', quantityFormula: 'Math.max(4, Math.ceil(widthMm / 300))', unit: 'each' },
      { name: 'Blum TANDEMBOX Drawer Runners', searchTerm: 'Blum TANDEMBOX drawer runner 400mm', quantityFormula: 'Math.max(1, Math.ceil(widthMm / 800))', unit: 'each' },
      { name: 'Engineered Stone Top', searchTerm: 'Engineered stone vanity top 20mm cut and polished', quantityFormula: '1', unit: 'each' },
      { name: 'Undermount Basin', searchTerm: 'Undermount ceramic basin 400 x 300mm', quantityFormula: 'basins', unit: 'each' },
      { name: 'Wall Mount Brackets', searchTerm: 'Wall hung vanity bracket heavy duty', quantityFormula: '2', unit: 'each' },
      { name: 'Cabinet Handles', searchTerm: 'Cabinet handle 128mm brushed nickel', quantityFormula: 'Math.ceil(widthMm / 400)', unit: 'each' },
      { name: 'Edging Tape', searchTerm: 'Iron on edging tape white 22mm x 10m', quantityFormula: '1', unit: 'each' },
      { name: 'Silicone Sealant', searchTerm: 'Silicone sealant neutral cure 300ml', quantityFormula: '1', unit: 'each' },
    ],
    estimatedHoursFormula: '(widthMm / 1000) * 6 + basins * 1',
    estimatedHoursRange: { min: 6, max: 18 },
    suggestedMaterials: ['Melamine carcass', 'Door fronts', 'Blum hinges', 'Drawer runners', 'Stone top', 'Undermount basin', 'Handles'],
    questionsLine: 'Width? Number of basins? Wall hung or floor standing? Top (stone, laminate)? Mixer customer-supplied?',
    exampleLines: '1200mm wall hung vanity, double undermount basin, 20mm engineered stone top with rolled edge. Polytec Woodmatt doors, two drawers with Blum soft-close. Mixers customer-supplied.',
    voicePromptHint: 'Tell me about the vanity — width, basins, mounting, top',
  },
  {
    id: 'tv_entertainment_unit',
    categoryId: 'cabinet_making',
    nicheId: 'entertainment_office',
    name: 'TV / Entertainment Unit',
    description: 'Custom built-in TV unit',
    icon: 'television',
    pricingMethod: 'per_linear_m',
    requiredParams: [
      { key: 'linearMetres', label: 'Unit Linear Metres', unit: 'lm', defaultValue: 3.5 },
      { key: 'height', label: 'Unit Height', unit: 'm', defaultValue: 2.4 },
    ],
    defaultMaterials: [
      { name: 'Melamine Carcass Board', searchTerm: 'White melamine board 2400 x 600 x 16mm', quantityFormula: 'Math.ceil(linearMetres * height * 0.7)', unit: 'each' },
      { name: 'Door / Drawer Fronts', searchTerm: 'Polytec laminated door panel custom size', quantityFormula: 'Math.ceil(linearMetres * 2)', unit: 'each' },
      { name: 'Blum Soft Close Hinges', searchTerm: 'Blum CLIP top BLUMOTION 110 hinge soft close', quantityFormula: 'Math.ceil(linearMetres * 4)', unit: 'each' },
      { name: 'Drawer Runners', searchTerm: 'Blum TANDEMBOX drawer runner 500mm', quantityFormula: 'Math.ceil(linearMetres)', unit: 'each' },
      { name: 'Cable Grommets', searchTerm: 'Cable grommet 60mm black', quantityFormula: 'Math.max(2, Math.ceil(linearMetres))', unit: 'each' },
      { name: 'Adjustable Shelves', searchTerm: 'Melamine shelf 600 x 300 x 16mm', quantityFormula: 'Math.ceil(linearMetres * 1.5)', unit: 'each' },
      { name: 'Shelf Pins', searchTerm: 'Shelf support pins nickel 5mm 100 pack', quantityFormula: '1', unit: 'pack' },
      { name: 'Handles / Finger Pulls', searchTerm: 'Cabinet handle 128mm brushed nickel', quantityFormula: 'Math.ceil(linearMetres * 2)', unit: 'each' },
      { name: 'Edging Tape', searchTerm: 'Iron on edging tape white 22mm x 50m', quantityFormula: '1', unit: 'each' },
      { name: 'Cabinet Screws', searchTerm: 'Cabinet screws 4mm x 30mm 500 pack', quantityFormula: '1', unit: 'pack' },
    ],
    estimatedHoursFormula: 'linearMetres * height * 2',
    estimatedHoursRange: { min: 8, max: 30 },
    suggestedMaterials: ['Melamine carcass', 'Door fronts', 'Blum hinges', 'Drawer runners', 'Cable grommets', 'Adjustable shelves', 'Handles'],
    questionsLine: 'Linear metres? Floor to ceiling or low unit? Doors / drawers / open shelving mix? Cable management for TV / AV?',
    exampleLines: '3.6 lm floor-to-ceiling TV unit in lounge. Centre TV recess, two flanking tall cupboards with adjustable shelving, low drawer bank under TV for AV gear. Polytec doors, push-to-open, cable grommets behind TV.',
    voicePromptHint: 'Tell me about the unit — linear metres, height, doors, AV',
  },
  {
    id: 'cabinet_door_replacement',
    categoryId: 'cabinet_making',
    nicheId: 'repairs_refacing',
    name: 'Cabinet Door Replacement',
    description: 'Replace existing cabinet doors and hardware',
    icon: 'door',
    pricingMethod: 'per_unit',
    requiredParams: [
      { key: 'doors', label: 'Number of Doors', unit: '', defaultValue: 12 },
      { key: 'drawers', label: 'Number of Drawer Fronts', unit: '', defaultValue: 4 },
    ],
    defaultMaterials: [
      { name: 'Polytec Door Panels', searchTerm: 'Polytec laminated door panel custom size', quantityFormula: 'doors', unit: 'each' },
      { name: 'Polytec Drawer Fronts', searchTerm: 'Polytec laminated drawer front custom size', quantityFormula: 'drawers', unit: 'each' },
      { name: 'Blum Soft Close Hinges', searchTerm: 'Blum CLIP top BLUMOTION 110 hinge soft close', quantityFormula: 'doors * 2', unit: 'each' },
      { name: 'Hinge Mounting Plates', searchTerm: 'Blum hinge mounting plate', quantityFormula: 'doors * 2', unit: 'each' },
      { name: 'Cabinet Handles', searchTerm: 'Cabinet handle 128mm brushed nickel', quantityFormula: 'doors + drawers', unit: 'each' },
      { name: 'Edging Tape', searchTerm: 'Iron on edging tape matching colour 22mm x 10m', quantityFormula: 'Math.ceil((doors + drawers) / 10)', unit: 'each' },
      { name: 'Cabinet Screws', searchTerm: 'Cabinet screws 4mm x 16mm 200 pack', quantityFormula: '1', unit: 'pack' },
    ],
    estimatedHoursFormula: '(doors * 0.4) + (drawers * 0.3)',
    estimatedHoursRange: { min: 4, max: 16 },
    suggestedMaterials: ['Door panels', 'Drawer fronts', 'Blum hinges', 'Mounting plates', 'Handles', 'Edging tape'],
    questionsLine: 'How many doors and drawer fronts? Existing carcass condition? Replace hinges and runners too? New finish (Polytec, 2-pac)?',
    exampleLines: '12 kitchen cabinet doors and 4 drawer fronts to replace. Existing carcasses in good condition, reuse. New Polytec Woodmatt doors, all new Blum soft-close hinges, customer-supplied handles.',
    voicePromptHint: 'Tell me about the refacing — doors, drawers, hardware, finish',
  },
