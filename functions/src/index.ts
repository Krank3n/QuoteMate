  ...
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      if (Array.isArray(userSavedRates) && userSavedRates.length > 0) {
        ...
        savedRatesSection = `\n\nUSER'S SAVED SUPPLIER RATES — PREFER THESE OVER RETAIL\nThe tradie has personal supplier rates below. These are alternative prices for materials they buy regularly — they are NOT a list of items to skip. You must still emit every required material in the "materials" array as normal; for any material that semantically matches a saved rate, tag the row with "savedRateName" and "pricingSource": "saved_rate" so the pricing layer uses the tradie's price instead of retail.\n\n${lines}\n\n*** CRITICAL — DO NOT OMIT MATCHED MATERIALS ***\nA saved rate is NOT a substitute for listing the material. If the job needs R2.5 insulation batts and the tradie has a saved Pink Batts R2.5 rate, you MUST output an insulation-batt material row (tagged with savedRateName), exactly as if there were no saved rate at all. The only difference between a matched row and an unmatched row is which pricing path the downstream layer takes. A job that needs insulation MUST contain an insulation line item — never reduce a quote to only PPE/consumables because the primary material had a saved rate.\n\nMatching rules:\n1. Match by meaning, not exact name. "concrete" matches a saved rate keyworded ["concrete","slab","footing"]. "R2.5 HD insulation batts" matches a saved "Pink Batts Wall R2.5 HD 1160x580x90 12PK" rate.\n2. If a rate has coveragePerUnit, that means one purchasable unit covers that much work-volume or work-area. Compute quantity = ceil(jobAmount / coveragePerUnit) where jobAmount is measured in coverageUnit. Examples: a sheet that covers 13 m² and a 40 m² wall → ceil(40/13) = 4 sheets; a batt-pack covering 8.08 m² and a 50 m² ceiling → ceil(50/8.08) = 7 packs; a mulch bag containing 0.5 m³ and a 2 m³ bed → ceil(2/0.5) = 4 bags. Always round UP — the tradie can't buy a fraction of a packaged unit.\n3. If the job gives an area but the rate is per m³ (e.g. ready-mix concrete sold loose by the m³ with no coveragePerUnit), pick a sensible slab thickness from job context (driveway ~125mm, residential slab ~100mm, footpath ~75mm) and explain your assumption in "reasoning". Compute m³ = area × thickness.\n4. For matched items set "savedRateName" to the exact saved rate name (copy it character-for-character from the list above) and "pricingSource": "saved_rate". Use the rate's unit for the row's "unit" field. Do NOT generate a retail searchTerm for these — leave searchTerm empty. But you MUST still include the row in "materials".\n5. Items with no matching saved rate flow through the normal retail pricing path — generate generic searchTerms for them as usual.\n6. When multiple saved rates could match (e.g. "Pink Batts Wall R2.5 HD" vs "Pink Batt Floor R2.5 HD" for a ceiling job), pick the one whose keywords/name best fits the application surface (ceiling/wall/floor) called out in the job description.\n`;
      }
...
- The PRIMARY material for the trade described in the job MUST always be present in the output. Insulation jobs must include batts; decking jobs must include boards; painting jobs must include paint; tiling jobs must include tiles; concreting jobs must include concrete/mix. A quote that contains only PPE, consumables, or tools without the primary material is invalid output — if you're tempted to omit the primary material because a saved rate exists, re-read the saved-rates section above: include it AND tag it with savedRateName.
