// Materials pipeline — orchestrates the "Get Recommended Gear" step:
//   1. generateMaterialsForQuote — the analyse pass: job description (+ plan
//      photos, templates, saved rates) → Material[] + QuoteSection[].
//   2. fetchPricesForQuote — the price fetch + reconcile orchestrator.
//
// This is the ONE implementation. It runs in two places:
//   - on the phone (src/services/materialsPipeline.ts binds the React Native
//     services and holds the screen awake), and
//   - inside a Cloud Function (functions/src/pricingRun.ts binds the server
//     internals) so a run survives the app being backgrounded or the phone
//     being locked.
// Everything environment-specific — the LLM calls, the supplier searches, the
// supplier-book reads, telemetry — arrives through PipelineDeps. Nothing here
// may import outside shared/ (see shared/pricing/types.ts for why).

import type {
  AnalyzeRequest,
  FavoriteProductMapping,
  Material,
  PricingBusinessSettings,
  PricingQuote,
  QuoteSection,
  ReconcileItem,
  ReconcileResult,
...
import type { LLMResponse } from './llmMaterials';
import type { EstimateResult } from './estimate';
import type { PriceFetchUsageSummary } from './priceFetchTelemetry';
 ...
 * server each supply one of these; the pipeline never touches a network,
 * a store or a native module itself.
 */
export interface PipelineDeps {
  /** The analyse LLM pass. Returns the normalised shape (see normaliseAnalyzeResponse). */
  analyzeJobDescription(request: AnalyzeRequest): Promise<LLMResponse>;
  /** One reconcile batch of at most RECONCILE_MAX_ITEMS_PER_REQUEST items. */
  reconcilePricedMaterials(
    items: ReconcileItem[],
    context: { jobName?: string; jobDescription?: string },
  ): Promise<ReconcileResult[]>;
  /** General-knowledge price estimate for one search term. */
  estimateMaterialPrice(term: string, hardwareStores: string[]): Promise<EstimateResult>;
  /** Up to five ranked Bunnings candidates for one term; [] on a miss. */
  searchBunningsCandidates(term: string): Promise<ScraperProduct[]>;
  /** One chunk of a Bunnings batch search — see BatchChunkFetcher. */
  batchSearchBunnings: BatchChunkFetcher;
  /** Reece candidates for one term; [] when Reece has nothing. The
   *  reauth/notConnected flags ride on the first element. */
  searchReeceCandidates(term: string): Promise<ReeceCandidate[]>;
  isReeceConnected(): Promise<boolean>;
  loadSupplierGroups(): Promise<SupplierGroup[]>;
  /** Every favourite keyed by its slug — the supplier book. */
  loadFavorites(): Promise<Record<string, FavoriteProductMapping>>;
  /** Favourites flagged isPersonalRate, for the analyse prompt. */
  loadPersonalRates(): Promise<FavoriteProductMapping[]>;
  /** Section templates, for the local pre-pricing pass. */
  loadTemplates(): Promise<SectionTemplate[]>;
 ...
 * in step with the cap in functions/src/index.ts (reconcilePricedMaterials).
 */
export const RECONCILE_MAX_ITEMS_PER_REQUEST = 50;

/**
 * Reconcile in server-sized batches, sequentially — the endpoint is
 * rate-limited per user and firing five at once trades a 400 for a 429.
 * Unbatched, a quote with more than 50 priceable rows 400'd, the caller's
 * catch swallowed it, and the whole safety pass silently skipped exactly the
 * big, high-value quotes where it matters most.
 */
async function reconcileInBatches(
  ...
  for (let i = 0; i < items.length; i += RECONCILE_MAX_ITEMS_PER_REQUEST) {
    ...
    results.push(...(await deps.reconcilePricedMaterials(batch, context)));
  }
  return results;
}
...
import { matchEvidence, stampMatchConfidence } from './matchEvidence';
...
export interface GenerateMaterialsArgs<Q extends PricingQuote = PricingQuote> {
  quote: Q;
  businessSettings: PricingBusinessSettings | null;
...
export interface GenerateMaterialsResult<Q extends PricingQuote = PricingQuote> {
 ...
 */
export async function generateMaterialsForQuote<Q extends PricingQuote>(
  deps: PipelineDeps,
  args: GenerateMaterialsArgs<Q>,
  ...
  const photoUrlsForAi: string[] | undefined = (() => {
    ...
    const urls = photos.map((p) => p.storageUrl).filter((u): u is string => !!u);
  ...
  const savedRateFavorites = await deps.loadPersonalRates();
  ...
  const analysis = await deps.analyzeJobDescription({
    jobDescription: quote.job.description,
    tradeContext,
    photoUrls: photoUrlsForAi,
  ...
  const localFavoritesById = await deps.loadFavorites();
...
export interface FetchPricesArgs<Q extends PricingQuote = PricingQuote> {
  quote: Q;
  businessSettings: PricingBusinessSettings | null;
  /**
...
export interface FetchPricesResult<Q extends PricingQuote = PricingQuote> {
  updatedQuote: Q;
  fetchedCount: number;
  failedCount: number;
  skippedCount: number;
...
export async function fetchPricesForQuote<Q extends PricingQuote>(
  deps: PipelineDeps,
  args: FetchPricesArgs<Q>,
  callbacks: FetchPricesCallbacks = {},
): Promise<FetchPricesResult<Q>> {
  ...
  if (liveReeceConnected === null) {
    try {
      liveReeceConnected = await deps.isReeceConnected();
    } catch {
  ...
  try {
    const supplierList = await deps.loadSupplierGroups();
    // The matcher is pure; its sources are read once per run, not once per row.
    const [localTemplates, localFavoritesMap] = await Promise.all([deps.loadTemplates(), deps.loadFavorites()]);
    const localFavorites = Object.values(localFavoritesMap);
  ...
  const reecePass = async (
    ...
    for (let i = 0; i < updatedMaterials.length; i++) {
      ...
      let candidates: ReeceCandidate[];
      try {
        candidates = await deps.searchReeceCandidates(term);
      } catch (err: any) {
        onEvent?.({
          kind: 'item-priced',
  ...
  try {
    ...
    if (remainingTerms.length > 0) {
      ...
      try {
        ...
        batchResults = await batchSearchProgressive(
          deps.batchSearchBunnings,
          remainingTerms,
          5,
          chunkSize,
    ...
    for (let i = 0; i < updatedMaterials.length; i++) {
      ...
      try {
        ...
        if (candidates.length === 0 && !nonRetailRowIds.has(material.id)) {
          candidates = await deps.searchBunningsCandidates(searchTerm);
        }
        if (candidates.length > 0) candidatesByMaterialId.set(material.id, candidates);
        ...
        try {
          const aiResult = await deps.estimateMaterialPrice(searchTerm, hardwareStores);
          if (aiResult.price) {
    ...
    if (reconcileItems.length > 0) {
      ...
      try {
        const results = await reconcileInBatches(deps, reconcileItems, {
      ...
      if (rejectedRows.length > 0) {
        try {
          ...
          for (const m of rejectedRows) {
            ...
            try {
              const cands = await deps.searchBunningsCandidates(simplified);
          ...
          if (rescueCands.size > 0) {
            ...
            const rescueResults = await reconcileInBatches(deps, rescueItems, {
              jobName: quote.job?.name,
              jobDescription: quote.job?.description,
            });
        ...
        for (const m of rejectedRows) {
          ...
          try {
            const aiResult = await deps.estimateMaterialPrice(m.searchTerm || m.name, hardwareStores);
            if (aiResult.price) {
    ...
    for (const m of updatedMaterials) {
      ...
      if (simplified && simplified !== term) {
        try {
          const retry = await deps.estimateMaterialPrice(simplified, hardwareStores);
          if (retry.price) {
      ...
      if (m.name && m.name !== simplified && m.name !== term) {
        try {
          const byName = await deps.estimateMaterialPrice(m.name, hardwareStores);
          if (byName.price) {
  ...
  try {
    deps.reportPriceFetchUsage?.(usageSummary);
  } catch {
